...
import (
	...
	genericutil "github.com/khulnasoft-labs/utils/generic"
	updateutils "github.com/khulnasoft-labs/utils/update"
)

var (
	defaultConfigLocation = filepath.Join(folderutil.HomeDirOrDefault("."), ".config/reveal/config.yaml")
)
...
func ParseOptions() *Options {
	...
	flagSet.CreateGroup("update", "Update",
		flagSet.CallbackVarP(GetUpdateCallback(), "update", "up", "update reveal to latest version"),
		flagSet.BoolVarP(&options.DisableUpdateCheck, "disable-update-check", "duc", false, "disable automatic reveal update check"),
	)
	...
	if !options.DisableUpdateCheck {
		latestVersion, err := updateutils.GetToolVersionCallback("reveal", version)()
		if err != nil {
			if options.Verbose {
				gologger.Error().Msgf("reveal version check failed: %v", err.Error())
			}
		} else {
			gologger.Info().Msgf("Current reveal version %v %v", version, updateutils.GetVersionDescription(version, latestVersion))
		}
	}
