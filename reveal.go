...
import (
	...
	"github.com/khulnasoft-labs/reveal/sources/agent/censys"
	"github.com/khulnasoft-labs/reveal/sources/agent/criminalip"
	"github.com/khulnasoft-labs/reveal/sources/agent/fofa"
	"github.com/khulnasoft-labs/reveal/sources/agent/hunter"
	"github.com/khulnasoft-labs/reveal/sources/agent/hunterhow"
	"github.com/khulnasoft-labs/reveal/sources/agent/netlas"
	"github.com/khulnasoft-labs/reveal/sources/agent/publicwww"
	"github.com/khulnasoft-labs/reveal/sources/agent/quake"
	"github.com/khulnasoft-labs/reveal/sources/agent/shodan"
	"github.com/khulnasoft-labs/reveal/sources/agent/shodanidb"
	"github.com/khulnasoft-labs/reveal/sources/agent/zoomeye"
...
type Options struct {
	Agents   []string // Reveal Agents to use
...
// Service handler of all reveal Agents
type Service struct {
	Options  *Options
...
// New creates new reveal service instance
...
func (s *Service) Execute(ctx context.Context) (<-chan sources.Result, error) {
	...
	switch {
	case len(s.Agents) == 0:
		return nil, errorutil.NewWithTag("reveal", "no agent/source specified")
	case !s.hasAnyAnonymousProvider() && !s.Provider.HasKeys():
		return nil, errorutil.NewWithTag("reveal", "agents %v requires keys but no keys were found", s.Options.Agents)
	}
...
func (s *Service) ExecuteWithCallback(ctx context.Context, callback func(result sources.Result)) error {
	...
	if callback == nil {
		return errorutil.NewWithTag("reveal", "result callback cannot be nil")
	}
	for {
		select {
		case <-ctx.Done():
...
// AllAgents returns all supported reveal Agents
...
func (s *Service) nilCheck() error {
	if s.Provider == nil {
		return errorutil.NewWithTag("reveal", "provider cannot be nil")
	}
	if s.Options == nil {
		return errorutil.NewWithTag("reveal", "options cannot be nil")
	}
	if s.Session == nil {
		return errorutil.NewWithTag("reveal", "session cannot be nil")
	}
	return nil
}
